...
1. **Empty droplets and ambient gene expression**. [`DropletUtils`](https://bioconductor.org/packages/release/bioc/vignettes/DropletUtils/inst/doc/DropletUtils.html) is an R package for filtering droplet-based scRNA-seq data. (See example use in [Brüning et al. (2022)](https://academic.oup.com/gigascience/article/doi/10.1093/gigascience/giac001/6515741)). This package can be used to read in the count matrix to R, filter empty droplets and visualise QC plots for the distribution of counts across barcodes, as well as downsample the count matrix or raw reads etc. However, more sophisticated tools make use of empty droplets to model **ambient gene expression**, which refers to contaminating cell-free RNA from other lysed cells.
...
SoupX quantifies cell-free RNA contamination from the profiles of empty droplets and outputs background-corrected counts for the remaining GEMs. DEIM [(Alvarez et al. 2020](https://www.nature.com/articles/s41598-020-67513-5) is another method designed to correct for ambient gene expression in single-nuclei RNA data.

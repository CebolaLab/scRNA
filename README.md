...
1. **Empty droplets and ambient gene expression**. A proportion of the barcodes in the count matrix will correspond to empty droplets. However, these empty droplets can contain contaminating cell-free RNA, known as *ambient gene expression*. This same cell-free RNA can contaminate droplets containing single-cells, leading to errors in downstream analysis, particularly if such RNA includes highly expressed marker genes used to identify cell-specific clusters. Several sophisticated methods exist with which to remove empty droplets and correct the remaining data for ambient or 'background' gene expression. Tools include SoupX [(Young and Behjati, 2020)](https://doi.org/10.1093/gigascience/giaa151), EmptyDrops [(Lun et al. 2019)](https://doi.org/10.1186%2Fs13059-019-1662-y) and DEIM [(Alvarez et al. 2020](https://www.nature.com/articles/s41598-020-67513-5) (note DEIM was designed for single-nuclei data, although shown to work with scRNA as well).
